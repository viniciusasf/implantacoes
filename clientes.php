...
$integracao = 0; $operacional = 0; $finalizacao = 0; $critico = 0; $encerrados = 0;
$clientes_filtrados = [];
            ...
            <?php else: ?>
                <?php foreach ($clientes_filtrados as $index => $c): 
                    ...
                    if ($cliente_encerrado) {
                        ...
                        if ($dias <= 30) { $status = 'integracao'; $status_color = '#0dcaf0'; $status_label = 'Integração'; }
                        elseif ($dias <= 70) { $status = 'operacional'; $status_color = '#0d6efd'; $status_label = 'Operacional'; }
                        elseif ($dias <= 91) { $status = 'finalizacao'; $status_color = '#ffc107'; $status_label = 'Finalização'; }
                        else { $status = 'critico'; $status_color = '#dc3545'; $status_label = 'Crítico'; }
                    ...
                    <?php else: ?>
                        <?php foreach ($clientes_filtrados as $c): 
                            ...
                            if ($cliente_encerrado) {
                                ...
                                if ($dias <= 30) { $status = 'Integração'; $status_color = '#0dcaf0'; }
                                elseif ($dias <= 70) { $status = 'Operacional'; $status_color = '#0d6efd'; }
                                elseif ($dias <= 91) { $status = 'Finalização'; $status_color = '#ffc107'; }
                                else { $status = 'Crítico'; $status_color = '#dc3545'; }
                            } else {
                                $status = 'Concluído';
                                $status_color = '#06d6a0';
                            }
